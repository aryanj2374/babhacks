...
async function resellTicket(client, sellerWallet, buyerWallet, tokenId, resalePrice, ticketMetadata) {
  ...
  if (maxResales > 0 && currentResales >= maxResales) {
    throw new Error(
      `Anti-scalping: ticket has reached the maximum number of resales (${maxResales})`
    );
  }
  ...
  console.log(`  Anti-scalping checks passed: ${resalePrice}/${ticketMetadata.maxResalePrice} XRP, resale ${currentResales + 1}/${maxResales || '∞'}`);
  ...
  const sellOfferTx = {
    ...
    Account: sellerWallet.address,
    NFTokenID: tokenId,
    Amount: xrpl.xrpToDrops(resalePrice),
    Destination: buyerWallet.address,
    Flags: 1, // tfSellNFToken
  };
  ...
  const royaltyAmount = (proposedPrice * config.DEFAULT_ROYALTY_BPS / 100000).toFixed(4);
  console.log(`  Ticket resold! Royalty auto-paid to organizer: ${royaltyAmount} XRP`);
